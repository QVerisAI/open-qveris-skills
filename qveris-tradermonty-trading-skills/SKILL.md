...
- Put the monitoring interpretation and risk explanation before trace details. Put detailed `qveris_trace` in the appendix as a compact table or fenced JSON.
...
- Use `fixtures/qveris/fallback-output.json` as the reviewer-facing example for proxy-only market regime fallback.
