...
2. Read `references/qveris-direct-runtime-contract.md` before any live call and use its audited runtime when available. Resolve `QVERIS_BASE_URL` or desktop `QVERIS_API_BASE_URL`. Read `references/qveris-direct-tool-map.md` before choosing tool-type queries. Read `references/qveris-direct-data-quality-rubric.md` before accepting evidence. Read `references/qveris-direct-retry-policy.md` after any failed, rejected, thin, or truncated result.
...
5. Validate every required parameter against the selected tool metadata, then execute with exact structured values and an explicit `--validation` contract. Use `quote` for timestamped quotes and `records` for financial/news collections. Do not copy sample identity values into a real call.
