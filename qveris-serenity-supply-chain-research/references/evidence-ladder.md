...
Use this file to grade sources and keep current research claims grounded. Source type is not acceptance by itself: every item must also pass identity/topic, cutoff, retrieval, provenance, and independence checks.

`evidence_owner` means the original issuer, exchange, regulator, government body, standards body, or publisher that owns the underlying record. A QVeris execution ID, CAP name, mirror URL, or repeated retrieval is never a new independent owner.
...
- attach a stable accepted evidence ID and name the source type;
- separate confirmed facts from interpretation;
- mark weak claims as leads;
- never let weak evidence satisfy a required factor or scarce-layer independence gate.
