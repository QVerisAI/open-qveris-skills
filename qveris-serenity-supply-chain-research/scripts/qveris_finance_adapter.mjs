...
function explicitOrEquivalentValue(name, parameters, context) {
  ...
  if (name === "market") {
    ...
    if (typeof symbol === "string" && /\.(SH|SS|SZ|BJ)$/i.test(symbol)) return "CN";
  }
  return undefined;
}
...
function isExplicitAshareRequest(parameters, context) {
  ...
  const symbol = parameters.symbol ?? parameters.ticker ?? context.symbol ?? context.ticker;
  return typeof symbol === "string" && /^\d{6}\.(?:SH|SS|SZ|BJ)$/i.test(symbol.trim());
}
...
function isAShareSymbol(value) {
  return /^\d{6}(?:\.(?:SH|SS|SZ|BJ))?$/i.test(String(value).trim());
}

function marketEquivalent(left, right) {
  const aliases = { CHINA: "CN", A: "CN", ASHARE: "CN", "A-SHARE": "CN", SSE: "CN", SZSE: "CN", BSE: "CN", BJ: "CN", BEIJING: "CN" };
