  ...
  for (const plan of expandCallPlan(config.callPlan, opts)) {
    if (paidCalls >= opts.maxPaidCalls) {
      trace.push({
        ...
        role: plan.role,
        target: plan.target,
        reason: "paid call budget exhausted",
      });
      continue;
    }
...
export async function runSkill(config, opts) {
  ...
  if (opts.fixture) {
    fixture = await loadFixture(opts.fixture);
    ...
    if (config.validateOptions) config.validateOptions(opts, { mode });
  ...
  const analysis = config.analyze
    ? config.analyze({ opts, calls: callResult.calls, preflight: preflightResult, fixture, trace: callResult.trace })
