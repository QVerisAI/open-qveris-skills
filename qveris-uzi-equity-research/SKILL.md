...
- When LHB or flow is explicitly requested, pass the logical `qveris_finance.*` name through the public adapter so it resolves the live canonical CAP ID. If live catalog/detail resolution is unavailable, mark the layer `capability_unavailable` or `tool_runtime_missing`; never fall back to a dated static CAP ID.
