# 金融 Skill 候选逐项评估卡片（Round 2 + Round 3 合并版）

日期：2026-07-06
用途：合并 Round 2 与 Round 3 的追加扫描结果，供金融同学统一评审候选 skill/仓库是否有金融业务价值、是否值得引入 QVeris/Open Skills。
范围：保留原编号 30-72，保留每个候选的 GitHub 原始链接、License、最近活跃、候选能力、直接可用性、核心代码、外部依赖、安全风险、测试情况、QVeris 改造方式和优先级。

## 合并版优先评审索引

| 优先级 | 候选 | 建议金融同学重点看什么 |
|---|---|---|
| P0 | Longbridge Skills | US/HK/A 股财报、行情、基本面、组合、研究 taxonomy |
| P0 | SEC Filing Legal Decoder | 10-K/20-F/10-Q 法律语言转金融风险卡，证据链和测试较强 |
| P0 | Alphasift | A 股策略筛选、候选池、后验评估，适合因子筛选方向 |
| P1 | cnstock-cli | A/H/US/指数/基金行情、K 线、板块、市场宽度，CLI 边界清楚 |
| P1 | HHFinAi Earnings Analysis | 机构级 earnings analysis 框架，可补强财报 rubric |
| P1 | Daymade Financial Suite | 金融数据采集、A 股新闻、RavenPack/Bigdata/Gangtise 数据 pipeline |
| P1 | UZI-Skill | 个股深度分析维度丰富，但需要强合规改造 |
| P1 | Vibe-Trading | 因子、回测、Alpha Zoo、期权、多市场数据，交易风险高 |
| P1 | Schwagent | factor/fundamental/financial-statement/risk skill 完整，测试较多 |
| P1 | A-Share Skill | A 股实时、历史、事件、行业/概念、A+H、快讯数据层 |
| P1 | A-Stock Data | A 股全栈数据覆盖，适合补 QVeris 中国市场 data map |
| P1 | Stock Analytics Skill | 投资分析 skill taxonomy，覆盖股票、ETF、基金、债券、comps、credit、macro |
| P1 | 3-Statement Ultra | 三表模型质量门禁、公式联动、QC 和审计约束 |
| P2/暂不投入 | 短线买卖点、目标价、止损、券商下单、登录态抓取类 | 只保留方法或字段参考，不进入近期引入 |

## 使用建议

金融同学可以先评审上表中的 P0/P1 项，不需要逐条看完所有 P2。工程/产品后续再根据金融结论检查 license、测试、数据源替换成本和 QVeris 接入方式。
