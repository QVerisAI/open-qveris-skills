...
Scope: Ask Codex agents to read each converted `qveris-anthropic-financial-services` through `qveris-financial-analyst-skills` skill and answer realistic natural-language finance requests using live QVeris calls. This is not the earlier smoke script; it tests whether an agent can naturally invoke the skill, choose tools, handle failures, and produce traceable output.

## Overall Result

| Skill | Verdict | Natural-language usability |
|---|---|---|
| `qveris-anthropic-financial-services` | usable-with-warning | Earnings workflow runs, but actual-surprise and transcript tools failed. |
| `qveris-langalpha` | usable-with-warning | DCF audit runs, but risk-free rate and period alignment need strict handling. |
| `qveris-eodhd-claude-skills` | good | Company brief workflow works; still needs stale quote and target-field suppression. |
| `qveris-finance-skills` | usable-with-warning | Factor workflow runs with qualitative fallback, but several core numeric tools failed. |
| `qveris-investskill` | usable-with-warning | Filing/red-flag sketch works; raw 10-K and true XBRL evidence were not available. |
| `qveris-tech-earnings-deepdive` | usable-with-warning | Estimates/news/market fallback works; not a full segment/transcript deep dive when core tools fail. |
| `qveris-day1global-skills` | usable-with-warning | Memo skeleton works; FX, macro region, and research payloads need sanity checks. |
| `qveris-earnings-tracker` | usable-with-warning | Recap-prep works; calendar returned out-of-window rows and transcript/surprise failed. |
| `qveris-hhxg-market` | usable-with-warning | A-share snapshot works with index fallback; CN breadth/top movers/margin/flow quality is weak. |
| `qveris-tradermonty-trading-skills` | usable-with-warning | Regime review works with VIX/rates/proxy fallback; breadth/index/macro tools failed. |
| `qveris-financial-analyst-skills` | usable-with-warning | Filing index and fundamentals extraction work; structured XBRL and period alignment need guarding. |
